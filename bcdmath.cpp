...
#define BCD_CONST_ONEP4   (BCD_CONST_EXP1+1)
#define BCD_CONST_ROOTPI  (BCD_CONST_ONEP4+1)
#define BCD_CONST_BCDMIN  (BCD_CONST_ROOTPI+1)
#define BCD_CONST_ROOT2   (BCD_CONST_BCDMIN+1)
...
#ifdef ARDUINO
#define GET_CONST(_bcd, _c)  \
    memcpy_P(&_bcd._v, constTable + (_c), sizeof(BCDFloat));
#else
#define GET_CONST(_bcd, _c)  \
    memcpy(&_bcd._v, constTable + (_c), sizeof(BCDFloat));
#endif

static const Dig constTable[] PROGMEM = 
{
    ...
    { 1, 4000, 0, 0, 0, 0, 0, 1 },     // 1.4 (used in erf)
    { 1, 7724, 5385, 905, 5160, 2729, 8167, 1 },  // sqrt(PI)
    { 10, 0, 0, 0, 0, 0, 0, 5693 }, // BCD MIN = 1e-9999
    { 1, 4142, 1356, 2373, 950, 4880, 1689, 1 }, // sqrt(2)
};

#ifndef SMALL_INT

#define BCD2_CONST_EXP1 0
#define BCD2_CONST_LANCZOS 1
...
static const Dig2 constTable2[] = 
{
    ...
    { 
        2, 7182, 8182, 8459, 452, 3536, 287, 
        4713, 5266, 2497, 7572, 4709, 3699, 9595,7496,
        1 },
...
{
    BCD v;
    GET_CONST(v, BCD_CONST_PI2);
    return v;
}

static void horner(int cbase, BCD& x, int n, BCD& res)
{
    int i;
    int cn = cbase + n;
    res = 0;
...
static void sinPoly(const BCD& a, BCD& sa)
{
    ...
    BCD y = a*a;
    BCD py;
    horner(BCD_CONST_SINPOLY, y, 7, py);
    sa = a*(1+y*py);
}
...
static void cosPoly(const BCD& a, BCD& ca)
{
    ...
    * NOTES:
    ...
    BCD y = a*a;
    BCD py;
    horner(BCD_CONST_COSPOLY, y, 7, py);
    ca = 1+y*py;
}
 ...
 */
BCD _gammaFactorialSlow(const BCD& z)
{
    ...
    int tb;
    int i, j;
    BCD half; GET_CONST(half, BCD_CONST_HALF);
...
void sincos(const BCD& v, BCD* sinv, BCD* cosv)
{
     ...
     */
    BCD piby32; GET_CONST(piby32, BCD_CONST_PIBY32);
    BCD a32 = a/piby32;
    k = ifloor(a32);
    ...
    if (k > 0) 
    {
        ...
        BCD piby32a; GET_CONST(piby32a, BCD_CONST_PIBY32A);
        BCD piby32b; GET_CONST(piby32b, BCD_CONST_PIBY32B);
        //a = a - k*piby32a - k*piby32b;
        BCD kf((unsigned int)k);
        a -= kf*piby32a;
        a -= kf*piby32b;
    }

#ifdef TRIG_SPECIAL_CASES
    // special case tests
    // numbers very close to special values 

#define IS_SMALL(_v)  ((_v).isZero() || (_v).exponent() <= -5)

    int k1 = k;
    
    // are we npi/32 + eps;
    bool npi32 = IS_SMALL(a);
    ...
    if (k) // sap & cap = 0 for k = 0.
    {
        ...
        if (k < 8) 
        {
            GET_CONST(sk, BCD_CONST_SINTAB-1+k);
            GET_CONST(ck, BCD_CONST_COSTAB-1+k);
            //sina = sk*ca + ck*sa;
            //cosa = ck*ca - sk*sa;
            
            
            // these version used less stack under gcc
            sina = sk*ca;
            sina += ck*sa;
            cosa = ck*ca;
            cosa -= sk*sa;
        ...
        {
            GET_CONST(sk, BCD_CONST_SINTAB+15-k);
            GET_CONST(ck, BCD_CONST_COSTAB+15-k);
            //sina = ck*ca + sk*sa;
            //cosa = sk*ca - ck*sa;
    ...
    if (sinv)
    {
        ...
        if (neg ^ (q > 1)) sinv->negate(); // *sinv = -*sinv;
    }
    ...
    if (cosv)
    {
        if ((q&1) == 0)
        {
            if (!cap) cosPoly(a, *cosv);
            else *cosv = cosa;
        }
        else
        {
            if (!sap) sinPoly(a, *cosv); 
            else *cosv = sina;
        }

        if ((q == 1) | (q == 2)) cosv->negate(); // *cosv = -*cosv;
    }
}
...
BCD sinDeg(const BCD& v)
{
    BCD sinv;
    if (v.isSpecial()) return v;

    BCD a(360);
    BCD v1 = fmod(v, a)*pi2()/a;

    sincos(v1, &sinv, 0);
    return sinv;
}

BCD sinGrad(const BCD& v)
{
    BCD sinv;
    if (v.isSpecial()) return v;
...
BCD powfmod(const BCD& a, int4 n, const BCD& fm)
{
    // ASSUME n >= 0
    // ASSUME a >= 0, fm > 0
    if (n == 0) return 1;
...
BCD exp(const BCD& v)
{
    ...
    BCD ln2; GET_CONST(ln2, BCD_CONST_LN2);
    BCD n = trunc(v/ln2);
...
static BCD logPoly(const BCD& r)
{
    ...
    */
    ...
    BCD r2 = r*r;
    BCD pr;
    horner(BCD_CONST_LOGPOLY, r2, 4, pr);
    return (pr*r2 + 1)*r;
}
...
static BCD _log(const BCD& v, int4& p10, int4& p2, bool& neg)
{
    ...
    BCD2 a21 = a2 - 1;
    BCD half; GET_CONST(half, BCD_CONST_HALF);
    int k = ifloor(a21.asBCD()*64 + half); // round to nearest k
    ...
    if (k)
    {
        BCD lnck; GET_CONST(lnck, BCD_CONST_LOGCK + k -1);
        lna += lnck;
    }
    return lna;
}
...
BCD ln10constant()
{
    BCD v; GET_CONST(v, BCD_CONST_LN10);
    return v;
}

BCD halfConstant()
{
    BCD v; GET_CONST(v, BCD_CONST_HALF);
    return v;
}
...
BCD log(const BCD& v)
{
    ...
    if (!lv.isSpecial())
    {
        BCD ln10; GET_CONST(ln10, BCD_CONST_LN10);
        BCD ln2; GET_CONST(ln2, BCD_CONST_LN2);
        lv += p10*ln10 + p2*ln2;
        if (neg) lv.negate();
    }
...
BCD log10(const BCD& v)
{
    ...
    if (!lv.isSpecial())
    {
        BCD ln2Oln10; GET_CONST(ln2Oln10, BCD_CONST_LN2OLN10);
        BCD ln10; GET_CONST(ln10, BCD_CONST_LN10);
...
BCD atan(const BCD& v)
{
    ...
    if (v.isSpecial()) 
    {
        if (v.isInf()) 
        {
            BCD piby2; GET_CONST(piby2, BCD_CONST_PIBY2);
     ...
     */
    BCD atanlim; GET_CONST(atanlim, BCD_CONST_ATANLIM);
    int doubles = 0;
    while (a > atanlim) 
    {
        ++doubles;
...
BCD atanDeg(const BCD& v)
{
    if (v.isSpecial()) return v;
    return atan(v)*BCD(180)/pi();
}

BCD atanGrad(const BCD& v)
{
    if (v.isSpecial()) return v;
    return atan(v)*BCD(200)/pi();
}
...
BCD atan2(const BCD& y, const BCD& x)
{
    ...
    if (x.isZero()) 
    {
        ...
        if (x.isNeg()) 
        {
            BCD pi; GET_CONST(pi, BCD_CONST_PI);
            if (y.isNeg()) 
                r = r - pi;
            else 
                r = r + pi;
...
{
    return atan2(y, x)*BCD(180)/pi();
}

BCD atan2Grad(const BCD& y, const BCD& x)
{
    return atan2(y, x)*BCD(200)/pi();
}
...
BCD asinDeg(const BCD& v)
{
    if (v.isSpecial()) return v;
    if (v < -1 || v > 1) 
        return BCDFloat::nan();

    return asin(v)*BCD(180)/pi();
}

BCD asinGrad(const BCD& v)
{
    if (v.isSpecial()) return v;
    if (v < -1 || v > 1) 
        return BCDFloat::nan();

    return asin(v)*BCD(200)/pi();
}
...
BCD acosDeg(const BCD& v)
{
    ...
    if (v == 1) return 0;
    return acos(v)*BCD(180)/pi();
}

BCD acosGrad(const BCD& v)
{
    if (v.isSpecial()) return v;
    if (v < -1 || v > 1) 
        return BCDFloat::nan();
    ...
    if (v == 1) return 0;
    return acos(v)*BCD(200)/pi();
}

bool rationalise2(BCD x, const BCD& w, BCD& n, BCD& d)
{
    // using Valentin's DEC2FRAC

    BCD v,u,y,c,t,r, z, s;
    BCD maxd = BCD::pow10(12);
    bool neg = x.isNeg();
    if (neg) x.negate();
...
bool rationalise(const BCD& a, const BCD& b, int4* num, int4* den)
{
    ...
    if (b0 < BCD::epsilon(26)*a) return false;
    ...
    // scale up to find ints. will fit in int when b is 0.001 or so.
    while (b0 < 1) 
    {
        d0*=100;
        b0*=100;
...
BCD pow(const BCD& x, const BCD& y)
{
    if (y.isZero()) 
    {
        // NB: 0^0 is not mathematically defined (depending on who you ask)
        // but some modern calculators say 0^0 = 1.
        // for now, nan
    ...
    // consider x^y when x < 0
    if (x.isNeg())
    {
        // if y = p/q, we have x^(p/q) = [-(-x)^(1/q)]^p
        // which is real if q is odd.

        int4 p, q;
        if (rationalise(y, BCD::epsilon(3), &p, &q))
        {
            if (q & 1) // only if q is odd!
            {
                // now see if this is the same number
                BCD pf(p);
                BCD qf(q);
                if (fabs(y - pf/qf) < BCD::epsilon(25)*fabs(y))
                {
                    // yes, use integer powering
                    return pow(-exp(log(-x)/qf), p);
                }
            }
        }
    }
...
BCD alog(const BCD& y)
{
    ...
    {
        int4 n = ifloor(y);
        if (n) return pow(BCD(10), n);
        
        /* otherwise power is too large */
        return y.isNeg() ? 0 : BCDFloat::posInf();
    }

    return exp(ln10constant()*y);
}
...
BCD modtwopi(const BCD& a)
{
    BCD pi2; GET_CONST(pi2, BCD_CONST_PI2);
    if (a < pi2) return a;
...
BCD ln1p(const BCD& a)
{
     ...
     * results.
     */
    BCD pointzeroone; GET_CONST(pointzeroone, BCD_CONST_HUNDREDTH);
...
BCD gammaFactorial(const BCD& c)
{
    ...
    else 
    {
        ...
        //return _gammaFactorialSlow(c);
    }
}

BCD gammaln(const BCD& z)
{
    /* return ln(gamma(c))
     */

    /* WARNING: not very accurate, need to perform some range
     * reduction!!!!
     */

    if (z.isSpecial()) return z;
    ...
    if (c.isNeg()) 
    {
        ...
        _gammaFactorialAux(z1, t1, t2, s);
        return log(z2/s) - t1*log(t2) + t2;
    }
    else 
    {
        _gammaFactorialAux(c, t1, t2, s);
        return t1*log(t2) - t2 + log(s);
    }
}
#endif // !SMALL_INT
...
BCD asinh(const BCD& v)
{
    ...
    BCD r = log(sqrt(a*a + 1) + a);
    if (neg) r.negate();

    return r;
}

BCD acosh(const BCD& v)
{
    BCD r(0);
...
BCD difftimeBCD(int, int)
{
    // XXX todo!
    return 0;
}

BCD erfNonNeg(const BCD& x)
{
    BCD x2;
    int i, j;
    ...
    BCD onep4; GET_CONST(onep4, BCD_CONST_ONEP4);
    BCD rootpi; GET_CONST(rootpi, BCD_CONST_ROOTPI);
    if (x < onep4)
    {
        BCD s = x;
        BCD t, t1, s2;
    ...
    else 
    {
        BCD an, b, c, d, del, h;
        BCD half; GET_CONST(half, BCD_CONST_HALF);
        BCD min; GET_CONST(min, BCD_CONST_BCDMIN);
...
BCD normalProbability(const BCD& v)
{
    BCD root2; GET_CONST(root2, BCD_CONST_ROOT2);
    BCD half; GET_CONST(half, BCD_CONST_HALF);
    return (erf(v/root2) + 1)*half;
}

BCD npr(const BCD& n, const BCD& r)
{
    // do it the lame way
    return gammaFactorial(n)/gammaFactorial(n-r);
}

BCD ncr(const BCD& n, const BCD& r)
{
    return npr(n,r)/gammaFactorial(r);
}
